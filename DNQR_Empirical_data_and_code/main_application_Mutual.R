...
folder = "C:/Users/sprin/Desktop/Application20211108"
setwd(folder) 
getwd()
...
################
NetMutual = YNetData$NetMutual

for (fundnum in c(3, 5, 7)) {
  
  Net = matrix(0, nrow = Nfirm, ncol = Nfirm)
  Net[NetMutual >=fundnum] = 1
  Netdensity = nnzero(Net)/(Nfirm^2-Nfirm)
  Netdensity
  

## in case some row sums are zero
for (i in which(rowSums(Net)==0) )
{
  Net[i, sample(setdiff(1:Nfirm,i), 3)] = 1                                                               ### for those node, randomly select 3 followees
}
W = Net/rowSums(Net)                                                                                   ### row normalize bA
W = as(W, "dgCMatrix")
...
figname = paste(folder, '/Figure/', yeardata, 'mallfigparW', fundnum,'N', Nfirm, '.pdf', sep = "")
